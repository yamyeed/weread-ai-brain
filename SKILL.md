...
description: 微信读书 AI 阅读外脑 — 数据可视化看板（GitHub 绿墙、24h 脑波图、认知盲区分析、WeRead-MBTI 灵魂卡片）+ AI 深度分析（概念通解、PM 拆解、法务排雷、跨书上下文联动）+ 知识重构导出。连接微信读书官方 API Key（wrk-xxx），默认输出 Markdown 格式，可选生成精美 HTML 卡片看板。
version: 2.0.0
---
...
触发词：「生成看板」/「看板」/「dashboard」

在对话中直接输出包含六个模块的 Markdown 格式报告，方便追问和交互。

### 模式 B：HTML 卡片看板（可选）

触发词：「生成 HTML 看板」/「HTML 版本」/「卡片版」

根据下方「HTML 看板模板规范」生成完整 HTML 文件，填充数据后保存到用户桌面并打开浏览器。输出路径：优先 `~/Desktop/`，其次 `~/桌面/`，最后 `~/`。打开命令：macOS 用 `open`，Linux 用 `xdg-open`，Windows 用 `start`。
...
保存到用户桌面目录（`~/Desktop/` 或 `~/桌面/` 或 `~/`），然后执行 `open`（macOS）/ `xdg-open`（Linux）/ `start`（Windows）打开浏览器。
...
分析时自动拉取该书的**历史划线和想法**作为上下文，让 AI 分析不局限于当前片段，而是结合用户在整本书中的思考轨迹。

**流程**：
1. 确定目标书籍（用户指定书名 → 搜索获取 bookId；或未指定 → 取 `/shelf/sync` 中在读第一本）
2. 调用 `/book/bookmarklist` 获取该书划线
3. 调用 `/review/list/mine` 获取该书想法
4. 将相关上下文拼入分析 prompt，标注来源章节
...
跨书按核心概念聚合划线，输出 Obsidian 友好的 Markdown：
...
分析完成后，可导出为 Markdown 文件保存到 `~/Desktop/`：
...
| 触发词 | 能力 | 模块 |
|:--|:--|:--|
| "生成看板" / "看板" / "dashboard" | Markdown 完整看板（5 模块） | 可视化 |
| "生成 HTML 看板" / "卡片版" / "HTML 版本" | HTML 卡片看板并打开浏览器 | 可视化 |
| "阅读打卡" / "绿墙" | 12 周打卡绿墙 | 可视化 |
| "习惯分析" / "黄金时段" | 24h 脑波频段图 | 可视化 |
| "盲区分析" / "偏科" | 知识光谱 + 认知盲区诊断 | 可视化 |
| "阅读人格" / "分析我" / "MBTI" | WeRead-MBTI 灵魂卡片 | 可视化 |
| "分析" / "拆解" / "帮我分析" | 询问分析模板 → 分析指定/当前书 | AI 分析 |
| "概念通解" | 用概念通解模板分析 | AI 分析 |
| "PM 拆解" / "产品拆解" | 用 PM 拆解模板分析 | AI 分析 |
| "法务排雷" / "合规分析" | 用法务排雷模板分析 | AI 分析 |
| "跨书关联" / "关联分析" | 多本书笔记交叉分析 | AI 分析 |
| "导出笔记" / "语义笔记" | 跨书语义聚类 Markdown | 知识导出 |
| "导出分析" | 导出当前分析为 Markdown 文件 | 知识导出 |
...
1. **确定目标书**：
   ...
   - 用户未指定 → 调用 `/shelf/sync` 取在读第一本书的 bookId
2. **选择分析模板**：
   - 用户已通过触发词指定模板 → 直接使用
   - 用户只说"分析"/"拆解" → 询问选择模板（概念通解 / PM 拆解 / 法务排雷）
3. **拉取上下文**：
...
完成后说「生成看板」看数据，说「分析」让 AI 帮你深度思考！
```
...
- 上下文联动最多选取 8 条相关划线/想法，避免 prompt 过长。
- 导出文件统一保存到用户桌面目录（`~/Desktop/` 或 `~/桌面/` 或 `~/`），文件名格式 `weread-{类型}-{日期}.md`。
